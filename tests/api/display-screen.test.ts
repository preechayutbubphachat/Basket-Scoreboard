...
function publicPrivateMetadataPattern() {
  return /assigned_by_user_id|created_by_user_id|updated_by_user_id|createdBy|updatedBy|homeTeamId|awayTeamId|playerId|playerFouls|roster|"(?:seq|sequence)"|currentSeq|lastEventSeq|seqNo|seq_no|eventSeq|eventSequence|projectionSeq|projectionSequence|last_event_seq|expectedSeq|projectionVersion|actor|device|session|token|csrf|password|authorization|commandId|correlationId|causationId|audit|correctionDetails|\/operator|\/admin|audit-log|replay|corrections/i;
}
